...
public class CheckoutBasketValidator : AbstractValidator<CheckoutBasketCommand>
{
    ...
    {
        RuleFor(x => x.BasketCheckoutDto).NotNull().WithMessage("BasketCheckoutDto cannot be null or empty");
        RuleFor(x => x.BasketCheckoutDto.UserName).NotEmpty().WithName("UserName is required");
    }
}
